...
from whatsapp_sender import (
    ...
    send_image_bytes_message, send_document_bytes_message,
)
...
from payments_store import save_payment_link
from notifications import notify
from human_takeover import get_conversation_state, request_handoff, log_message, contains_handoff_keyword
from odoo_connections import get_odoo_connection, get_ai_config
    ...
    for _ in range(ai_config["max_tool_rounds"]):
        ...
        for tool_call in message.tool_calls:
            ...
            if tool_name == "create_quotation":
                set_pending(phone_number_id, user_id, arguments)
                result = {
...
def process_whatsapp_message(job):
    """Runs in the worker. Handles one incoming WhatsApp message end to end:
    media processing, handoff/pending checks, run_agent(), sending the reply,
    logging. FastAPI's /webhook only validates and enqueues this job."""
    ...
    if message["type"] == "text":
        ...
        try:
            ...
            audio_path = download_media(media_id, phone_number_id, "ogg")
            user_message = transcribe_audio(audio_path, phone_number_id)
        ...
        try:
            ...
            image_path = download_media(media_id, phone_number_id, "jpg")
            description = describe_image(image_path, phone_number_id, caption)
        ...
        try:
            ...
            if mime_type != "application/pdf":
                send_text_message(user_id, "Sorry, I can only read PDF documents right now.", phone_number_id)
                return

            doc_path = download_media(media_id, phone_number_id, "pdf")
            extracted_text = extract_pdf_text(doc_path, phone_number_id)
            ...
            send_text_message(user_id, "Sorry, I couldn't process that document right now.", phone_number_id)
            return
    else:
        return
    ...
    try:
        convo_state = get_conversation_state(phone_number_id, user_id)
        print("CONVERSATION STATE:", convo_state)
    except Exception as e:
    ...
    if contains_handoff_keyword(user_message, ai_config["handoff_keywords"]):
        try:
            request_handoff(phone_number_id, user_id, "Customer asked for a human")
            log_message(phone_number_id, user_id, "inbound", "customer", user_message, message_id)
            send_text_message(
                user_id,
    ...
    if not load_history(user_id, phone_number_id):
        try:
            notify("welcome", user_id, phone_number_id, {})
        except Exception as e:
            logger.error("Welcome notification failed for %s: %r", user_id, e)

    pending = get_pending(phone_number_id, user_id)
    if pending:
        ...
        if word_count <= ai_config["confirm_max_words"] and any(w in lowered for w in confirm_words):
            ...
            arguments["phone_number_id"] = phone_number_id
            result = create_quotation(**arguments)
            clear_pending(phone_number_id, user_id)
            ...
            if result.get("created"):
                ...
                try:
                    link = create_payment_link(
                        phone_number_id, amount_total, currency,
                        f"WhatsApp Customer {user_id}", user_id,
                        f"Order {order_name}: {quantity} x {product_name}", order_name,
                    )
                    save_payment_link(link["payment_link_id"], phone_number_id, user_id, order_id, order_name)
                    context = {
                    ...
                    try:
                        notify("order_confirmed", user_id, phone_number_id, context)
                    except Exception as e:
                    ...
                    try:
                        notify("order_confirmed_no_link", user_id, phone_number_id, context)
                    except Exception as e:
                        logger.error("send_text_message failed for %s: %r", user_id, e)
            else:
                try:
                    send_text_message(user_id, "Sorry, I couldn't create that order — please try again.", phone_number_id)
                except Exception as e:
                    logger.error("send_text_message failed for %s: %r", user_id, e)
            return
        ...
        if word_count <= ai_config["confirm_max_words"] and any(w in lowered for w in cancel_words):
            ...
            try:
                send_text_message(user_id, reply, phone_number_id)
            except Exception as e:
                logger.error("send_text_message failed for %s: %r", user_id, e)
            return
    ...
    if any(kw in lowered_msg for kw in IMAGE_KEYWORDS):
        ...
        if candidate_name:
            try:
                details = get_product_details(candidate_name, phone_number_id)
            except Exception as e:
            ...
            if details.get("found") and details.get("has_image") and details.get("image_base64"):
                try:
                    ...
                    send_image_bytes_message(
                        user_id, image_bytes, phone_number_id,
                        caption=f"📸 {details['product_name']}"
                    )
                    log_message(phone_number_id, user_id, "inbound", "customer", user_message, message_id)
                    log_message(phone_number_id, user_id, "outbound", "ai", f"Sent photo of {details['product_name']}")
    ...
    try:
        reply, product_image_base64 = run_agent(user_id, user_message, phone_number_id)
        print("AFTER RUN_AGENT:", reply)
    except Exception as e:
        print("RUN_AGENT ERROR:", repr(e))
    ...
    print("CALLING SEND_TEXT")

    try:
        send_text_message(user_id, reply, phone_number_id)
        print("SEND_TEXT RETURNED")
    except Exception as e:
    ...
    try:
        log_message(phone_number_id, user_id, "outbound", "ai", reply)
    except Exception as e:
        logger.error("log_message failed for %s: %r", user_id, e)


def process_broadcast_job(job):
    """Runs in the worker. Same send logic /internal/broadcast/send used to
    run inline — moved here so FastAPI doesn't block on N recipients."""
    phone_number_id = job["phone_number_id"]
    ...
    for r in recipients:
        ...
        try:
            ...
            if r.get("send_mode") == "text":
                response = send_text_message(r["phone"], r["text_body"], phone_number_id)
            else:
            ...
            if isinstance(response, dict) and "error" in response:
                logger.error("Broadcast send failed for recipient %s: %s", recipient_id, response["error"])
            else:
                logger.info(
                    "Broadcast sent to recipient %s (whatsapp_message_id=%s)",
                    recipient_id, whatsapp_message_id
                )
        except Exception as e:
